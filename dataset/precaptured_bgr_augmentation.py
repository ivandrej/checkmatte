...
"""
    Applies multiple augmentations to the precaptured bgr only, and not the bgr behind the person.
    The goal is to simulate different types of mis-alignment between the bgr and the person video, e.g rotations
"""
class PrecapturedBgrOnlyAugmentation:
    def __init__(self,
                 size,
                 prob_fgr_affine,
                 prob_bgr_affine,
    ...
    def __call__(self, fgrs, phas, bgrs, bgrs_):
        # Motion affine
        if random.random() < self.prob_bgr_affine:
            bgrs_ = MotionAugmentation.motion_affine(bgrs_, degrees=(-20, 20), shears=(-8, 8))

        # Still Affine
        if self.static_affine:
            bgrs_ = MotionAugmentation.static_affine(bgrs_, degrees=(-15, 15), scale_ranges=(1, 1.5))
        ...
        # Noise
        if random.random() < self.prob_noise:
            bgrs_ = MotionAugmentation.motion_noise(bgrs_)

        # Color jitter
        if random.random() < self.prob_color_jitter:
            bgrs_ = MotionAugmentation.motion_color_jitter(bgrs_)
        ...
        if random.random() < self.prob_blur:
            bgrs_ = MotionAugmentation.motion_blur(bgrs_)

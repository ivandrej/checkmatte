...
from composite import fgr_paths, pha_paths, bgrs_per_fgr
...
class Evaluator:
    ...
    def evaluate_worker(self, dataset, clip, position):
        print("Clip, position: ", clip, position)
        input()
        true_fgr_frames = pims.PyAVVideoReader(fgr_paths[int(position / bgrs_per_fgr)])
        true_pha_frames = pims.PyAVVideoReader(pha_paths[int(position / bgrs_per_fgr)])
...
# TODO: Move metrics to separate class
class MetricMAD:

...
sys.path.append('..')
from model import model, model_attention_f3
...
for clip in os.listdir(input_dir):
    outdir = os.path.join("/media/andivanov/DATA/results/", args.clipgroup, clip, 'rvm')
...
model.load_state_dict(torch.load("/media/andivanov/DATA/euler_training/roughly_matched/attention_variants/"
                                 "f3/finetuned/offset10/res128_lr1e4_B4/checkpoint/stage1/epoch-27.pth"))

for clip in os.listdir(input_dir):
    outdir = os.path.join("/media/andivanov/DATA/results/", args.clipgroup, clip, 'attention_f3_offset10')
